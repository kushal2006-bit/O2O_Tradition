...
o2oCsrfToken();requireLogin('customer','login.php');$db=getDB();$id=(int)$_SESSION['customer_id'];$error='';$success='';$deletionRequested=false;
...
$ds=$db->prepare("SELECT id,status,requested_at FROM customer_deletion_requests WHERE customer_id=? AND status='requested' ORDER BY id DESC LIMIT 1");$ds->execute([$id]);$deletionRequest=$ds->fetch();$deletionRequested=(bool)$deletionRequest;
...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if($action==='profile'){
  ...
  $reason=trim($_POST['deletion_reason']??'');$st=$db->prepare("SELECT id FROM customer_deletion_requests WHERE customer_id=? AND status='requested' LIMIT 1");$st->execute([$id]);
  if($st->fetch())$error='A deletion request is already pending review.';else{$db->prepare("INSERT INTO customer_deletion_requests(customer_id,reason) VALUES(?,?)")->execute([$id,$reason!==''?$reason:null]);$deletionRequested=true;$success='Your account deletion request has been submitted for review.';}
...
<div class="nav"><b>O2O Tradition</b><div><a href="home.php">Home</a><a href="privacy.php">Privacy</a><a href="orders.php">Orders</a><a href="logout.php">Logout</a></div></div><main class="main"><h1>Account Settings</h1><?php if($success):?><div class="ok"><?=htmlspecialchars($success)?></div><?php endif;?><?php if($error):?><div class="err"><?=htmlspecialchars($error)?></div><?php endif;?>
...
<section class="card danger"><h2>Request Account Deletion</h2><p class="small">Request permanent personal-data erasure. Transaction records may be retained where required for marketplace, payment, fraud, accounting, or dispute records; approved requests replace your personal account details with an anonymized record.</p><?php if($deletionRequested):?><p class="small"><b>Deletion request pending review.</b> Submitted <?=htmlspecialchars($deletionRequest['requested_at'])?>.</p><?php else:?><form method="POST"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars($_SESSION['csrf_token'])?>"><input type="hidden" name="action" value="request_deletion"><div class="field"><label>Reason (optional)</label><textarea name="deletion_reason" rows="3"></textarea></div><button class="btn" type="submit">Request Account Deletion</button></form><?php endif;?></section><section class="card danger"><h2>Deactivate Account</h2><p class="small">Deactivation blocks future sign-in while retaining order and marketplace records needed for transaction history. This is not irreversible deletion.</p><form method="POST"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars($_SESSION['csrf_token'])?>"><input type="hidden" name="action" value="deactivate"><div class="field"><label>Confirm with password</label><input type="password" name="deactivate_password" required></div><button class="btn" type="submit">Deactivate My Account</button></form></section>
</main></body></html>