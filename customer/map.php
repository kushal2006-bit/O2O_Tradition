...
const stores=<?=json_encode(array_map(function($v){return ['id'=>(int)$v['id'],'name'=>$v['store_name'],'address'=>$v['address']??'','lat'=>$v['latitude']!==null?(float)$v['latitude']:null,'lng'=>$v['longitude']!==null?(float)$v['longitude']:null,'count'=>(int)$v['item_count'],'pincode'=>$v['pincode']??''];},$vendors),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)?>;
...
const requestedPincode=<?=json_encode(substr($pincode,0,3))?>;
function pincodeMatch(s){return !requestedPincode || String(s.pincode||'').startsWith(requestedPincode);}
function showPincodeResults(){originalStores.forEach(s=>{const card=document.querySelector('.store[onclick="focusStore('+s.id+')"]');if(card)card.style.display=pincodeMatch(s)?'':'';if(markers[s.id]&&!pincodeMatch(s)&&map.hasLayer(markers[s.id]))map.removeLayer(markers[s.id]);});}
function applyRadius(lat,lng){const radius=parseFloat(document.getElementById('radius').value)||5;let visible=0;let nearest=[];originalStores.forEach(s=>{const card=document.querySelector('.store[onclick="focusStore('+s.id+')"]');if(s.lat===null||s.lng===null){if(card)card.style.display='none';return;}const km=distanceKm(lat,lng,s.lat,s.lng);const e=document.getElementById('distance-'+s.id);if(e)e.textContent=km.toFixed(1)+' km from you';const inside=km<=radius && pincodeMatch(s);if(card)card.style.display=inside?'':'none';if(markers[s.id]){if(inside){if(!map.hasLayer(markers[s.id]))markers[s.id].addTo(map);nearest.push([s.lat,s.lng]);visible++;}else if(map.hasLayer(markers[s.id]))map.removeLayer(markers[s.id]);}});document.getElementById('locationNote').textContent=visible?'Showing '+visible+' store(s) within '+radius+' km. Location was used locally and not saved.':'No stores with coordinates were found within '+radius+' km. Try a larger radius or pincode search.';if(nearest.length)map.setView([lat,lng],13);}
