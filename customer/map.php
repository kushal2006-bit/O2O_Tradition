...
<section class="card"><form method="GET" class="controls"><input name="pincode" value="<?=htmlspecialchars($pincode)?>" placeholder="Pincode"><select name="mode"><option value="all" <?=$mode==='all'?'selected':''?>>Rent & Buy</option><option value="rent" <?=$mode==='rent'?'selected':''?>>Rent</option><option value="buy" <?=$mode==='buy'?'selected':''?>>Buy</option></select><button type="submit">Find stores</button><button type="button" id="locate">Use my location</button></form><p class="small" id="locationNote">Location is optional and is not saved by this page.</p></section>
...
document.getElementById('locate').addEventListener('click',()=>{const n=document.getElementById('locationNote');if(!navigator.geolocation){n.textContent='Location is unavailable in this browser.';return;}n.textContent='Requesting location…';navigator.geolocation.getCurrentPosition(pos=>{let found=0;stores.forEach(s=>{if(s.lat===null||s.lng===null)return;found++;const e=document.getElementById('distance-'+s.id);if(e)e.textContent=distanceKm(pos.coords.latitude,pos.coords.longitude,s.lat,s.lng).toFixed(1)+' km from you';});L.circleMarker([pos.coords.latitude,pos.coords.longitude],{radius:7}).addTo(map).bindPopup('Your approximate location');n.textContent=found?'Distances calculated locally; your location was not saved.':'No store coordinates are available yet.';if(found)map.setView([pos.coords.latitude,pos.coords.longitude],12);},()=>{n.textContent='Location permission was not granted. Use the pincode filter instead.';},{enableHighAccuracy:false,maximumAge:300000,timeout:10000});});
</script></body></html>