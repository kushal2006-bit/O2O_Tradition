...
if($_SERVER['REQUEST_METHOD']==='POST'&&($_POST['action']??'')==='save_location'){
  ...
  if($lat===''&&$lng===''){
    $db->prepare("UPDATE vendors SET latitude=NULL,longitude=NULL WHERE id=?")->execute([$vendorId]);
  }elseif(is_numeric($lat)&&is_numeric($lng)&&abs((float)$lat)<=90&&abs((float)$lng)<=180){
    $db->prepare("UPDATE vendors SET latitude=?,longitude=? WHERE id=?")->execute([(float)$lat,(float)$lng,$vendorId]);
  }
