...
o2oCsrfToken();requireLogin('vendor','login.php');$db=getDB();$vendorId=$_SESSION['vendor_id'];$storeName=$_SESSION['vendor_name'];
...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['action'])){o2oRequireCsrf();$orderId=intval($_POST['order_id']??0);if($_POST['action']==='confirm'){$q=$db->prepare("SELECT customer_id,item_id,status FROM orders WHERE id=? AND vendor_id=?");$q->execute([$orderId,$vendorId]);$r=$q->fetch();if($r&&$r['status']==='new'){\n  $updated=$db->prepare("UPDATE orders SET status='in_progress' WHERE id=? AND vendor_id=? AND status='new'")->execute([$orderId,$vendorId]);\n  if($updated)addCustomerNotification($db,(int)$r['customer_id'],'rental_status','Rental is active','Your rental order #'.str_pad($orderId,6,'0',STR_PAD_LEFT).' is confirmed and now active.');\n}}elseif($_POST['action']==='complete'){$actual=$_POST['actual_return_date']??date('Y-m-d');$st=$db->prepare("SELECT o.*,i.late_charge_per_day FROM orders o JOIN items i ON o.item_id=i.id WHERE o.id=? AND o.vendor_id=?");$st->execute([$orderId,$vendorId]);$o=$st->fetch();if($o){$lateDays=max(0,(int)ceil((strtotime($actual)-strtotime($o['return_date']))/86400));$lateCharge=$lateDays*$o['late_charge_per_day'];$db->beginTransaction();try{
